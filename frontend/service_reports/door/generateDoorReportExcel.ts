import { DoorCustomerInfo, DoorReportData, DoorTimeSpent } from '@/types/doorReportTypes';
import { generateServiceReportExcel, writeMeasurementTable } from '@/service_reports/common/serviceReportExcel';

export async function generateDoorReportExcel(
  ...
  await generateServiceReportExcel({
    title: 'Service Report Rolling Door',
    equipmentLabel: 'Door',
    customerInfo: {
    ...
    photos,
    fileName: `Service_Report_Door_${customerInfo.serialNo || 'Draft'}`,
    writeMeasurements: (ws, startRow) => {
      ...
      row = writeMeasurementTable(ws, row, 'NOISE & GROUNDING',
        ...
        [
          ['Noise Unit', reportData.noise.unitResult, reportData.noise.standard, reportData.noise.remarks],
          ['Grounding', reportData.grounding.groundingResult, reportData.grounding.standard, reportData.grounding.remarks],
        ]
      );
      row++;
      return row;
