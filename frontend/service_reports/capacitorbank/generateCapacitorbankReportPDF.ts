...
export async function generateCapacitorbankReportPDF(
  ...
  doc.text('SERVICE REPORT PANEL AUTOMATIC POWER FACTOR CORECTION RELAY', pageWidth / 2, currentY + 4, { align: 'center' });
  ...
  const custTableData = [
    ...
    [
      ...
      customerInfo.equipmentName || 'Panel APFCR',
  ...
  // Section: Cleaning (5 Items)
  doc.setFillColor(30, 58, 138);
  doc.rect(margin, currentY, contentWidth, 4.5, 'F');
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(7);
  doc.setTextColor(255, 255, 255);
  doc.text('Cleaning   Please mark OK ( √ ), not OK ( × ), not applicable (N/A) in the box', margin + 2, currentY + 3.2);
  currentY += 4.5;
  ...
  doc.setFontSize(7);
  doc.setTextColor(255, 255, 255);
  doc.text('Capacitance Measurement   Please mark OK ( √ ), not OK ( × ), not applicable (N/A) in the box', margin + 2, currentY + 3.2);
  currentY += 4.5;

  autoTable(doc, {
    startY: currentY,
    head: [['Wire', 'Result (uF)', 'Standard', 'Remarks']],
    body: [['Capacitance', reportData.capacitance.capacitanceResult || '-', reportData.capacitance.standard, reportData.capacitance.remarks || '-']],
    theme: 'grid',
    headStyles: { fillColor: [241, 245, 249], textColor: [15, 23, 42], fontStyle: 'bold', fontSize: 6 },
    styles: { fontSize: 5.5, cellPadding: 1, textColor: [30, 41, 59] },
    columnStyles: {
  ...
  const analysisBody = [
    ['Normal operation', `Remark : ${reportData.analysis.remark || 'Panel APFCR beroperasi normal dan faktor daya terjaga.'}`],
  ...
  // Save PDF
  const filename = `Service_Report_Capacitor_Bank_${customerInfo.companyName.replace(/\s+/g, '_')}_${timeSpent.date || 'draft'}.pdf`;
  doc.save(filename);
}
