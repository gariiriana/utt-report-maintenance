...
export function CorrectiveMaintenance({ readOnly = false, initialSearchQuery }: CorrectiveMaintenanceProps) {
    ...
    const explicitSparepartCMIds = new Set(
        allCMReports
            .filter(cm => (cm as any).troubleshootType === 'sparepart_replacement' || cm.isSparepartReplacement === true)
            .map(cm => cm.id)
    );
                ...
                <>
                    ...
                    {!loading && (
                        <div className="mb-6 bg-white/90 backdrop-blur-xl border border-slate-200 rounded-2xl p-4 flex flex-col md:flex-row gap-4 items-center justify-between shadow-sm">
                            <div className="flex flex-col sm:flex-row gap-2.5 items-center w-full md:w-auto flex-wrap">
                                ...
                                <div className="flex items-center gap-1.5 w-full sm:w-auto">
                                    ...
                                    <select
                                        ...
                                        className="w-full sm:w-auto px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 text-sm font-semibold focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none transition shadow-sm cursor-pointer"
                                    >
                                    ...
                                    <div className="relative inline-flex items-center shrink-0">
                                        ...
                                        <button
                                            type="button"
                                            tabIndex={-1}
                                            className="p-2.5 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl text-slate-500 hover:text-orange-600 transition shadow-sm cursor-pointer flex items-center justify-center shrink-0"
                                            title="Pilih langsung dari kalender"
                                        >
                                ...
                                {(selectedDay !== 'all' || selectedMonth !== 'all' || selectedYear !== 'all' || searchQuery.trim() !== '' || adminDeleteFilter !== 'all') && (
                                    <button
                                        type="button"
                                        onClick={() => {
                                            setSelectedDay('all');
                                        ...
                                        className="px-2.5 py-2 text-xs font-bold text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-xl border border-slate-200 hover:border-red-200 transition cursor-pointer flex items-center gap-1 shrink-0 bg-white shadow-xs"
