...
export const PM_SCHEDULE_DATA: PMScheduleItem[] = [
 ...
 */
export async function getWAReminderConfig(): Promise<WAReminderConfig> {
  ...
  const data = snap.data() as WAReminderConfig;
  return {
    ...data,
 ...
 */
export async function saveWAReminderConfig(config: Partial<WAReminderConfig>, updatedBy: string): Promise<void> {
  const db = admin.firestore();
  const docRef = db.collection('system_status').doc('wa_reminder_config');
 ...
 */
export async function logWAReminderAudit(log: {
 ...
 */
export function buildH60Message(targetMonthIndex: number): { message: string; pmCount: number; monthName: string } {
