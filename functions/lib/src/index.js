...
function getNextAPIKey() {
    if (apiKeys.length === 0) {
        throw new https_1.HttpsError("failed-precondition", "NVIDIA NIM API Keys are not configured in process.env.");
    }
...
// ─── NVIDIA API Caller ──────────────────────────────────────────────────────
async function callNVIDIA(apiKey, model, messages, temperature = 0.0, maxTokens = 4096, extraBody = {}) {
    const payload = {
        model,
        messages,
    ...
    if (!response.ok) {
        ...
        throw new https_1.HttpsError("internal", `NVIDIA NIM API error (${response.status}): ${errText}`);
    }
    const data = await response.json();
    if (!data.choices || data.choices.length === 0) {
        throw new https_1.HttpsError("internal", "NVIDIA NIM API returned no choices");
    }
    return data.choices[0].message.content || "";
}
// ─── Category Vision Worker ──────────────────────────────────────────────────
// ─── Single Photo Vision Worker ──────────────────────────────────────────────
async function runSinglePhotoAnalysis(photo, index) {
...
exports.analyzeATSReport = (0, https_1.onCall)({ region: "asia-southeast1", cors: true, timeoutSeconds: 300 }, async (request) => {
    ...
    const photoPromises = photos.map((p, idx) => runSinglePhotoAnalysis(p, idx));
...
- For visual_inspection: keep the exact activity texts shown above, only update condition and remarks based on vision agent data`;
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", `Consolidation parsing failed: ${error.message}`);
    }
});
// ─── WhatsApp Cloud Gateway & H-60 Scheduled / Callable Functions ────────────
var scheduled_wa_reminder_1 = require("./scheduled-wa-reminder");
Object.defineProperty(exports, "scheduledWAReminderH60", { enumerable: true, get: function () { return scheduled_wa_reminder_1.scheduledWAReminderH60; } });
var manual_wa_send_1 = require("./manual-wa-send");
